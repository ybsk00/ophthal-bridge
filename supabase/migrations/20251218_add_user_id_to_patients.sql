...
-- 3. user_id 인덱스 생성 (빠른 조회용)
CREATE INDEX IF NOT EXISTS idx_patients_user_id ON public.patients(user_id);

-- 4. user_id 유니크 제약 추가 (한 유저는 하나의 환자 레코드만)
-- 이미 데이터가 있을 수 있으므로 UNIQUE 대신 partial unique index 사용
CREATE UNIQUE INDEX IF NOT EXISTS idx_patients_user_id_unique 
  ON public.patients(user_id) WHERE user_id IS NOT NULL;

-- 5. RLS 정책 업데이트 - 사용자는 자신의 환자 정보만 수정 가능
...
CREATE POLICY "Users can view own patient record" ON public.patients 
  FOR SELECT USING (auth.uid() = user_id OR true); -- 기존 정책 유지 (모든 사용자 읽기 가능)
